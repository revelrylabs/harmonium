...
import {HighlightsType} from './DatePicker/CalendarDay'
 ...
 */
function goodDateInput(): boolean {
...
class UncontrolledDatePicker extends React.Component<DatePickerProps, DatePickerState> {
  /**
   ...
   */
  valuesFromProps(props: DatePickerProps) {
  ...
  fireChangeHandler = () => {
    if (this.nativeInput) {
      ...
      this.onChange(event as unknown as SyntheticEvent)
    }
  }
   ...
   * @return {void}
   */
  getCalendarRef = (ref: HTMLDivElement | null) => {
    this.calendar = ref
  }
   ...
   * @return {void}
   */
  onOutsideClick = (event: React.MouseEvent) => {
    if (
      this.state.isOpen &&
      this.calendar &&
      !this.calendar.contains(event.target as Node)
    ) {
    ...
    return (
      ...
      >
        ...
        <DateInputBlock
          {...dateInputBlockProps}
          id={inputId}
          error={typeof error === 'boolean' || typeof error === 'string' ? error : undefined}
          ...
          inputRef={(input: HTMLInputElement | null) => (this.nativeInput = input)}
          overrides={overrides}
        />
