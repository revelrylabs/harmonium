import {includes} from 'lodash'

export interface S3Response {
  fileUrl: string;
  signedRequestUrl: string;
}

export type GetSignedUrlFunction = (file: File) => Promise<S3Response>;

// upload to a presigned s3 url
function makeS3Request(fileUrl: string, signedRequestUrl: string, file: File): Promise<string> {
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest()

    xhr.onreadystatechange = () => {
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          resolve(fileUrl)
        } else {
          reject({
            status: xhr.status,
            statusText: xhr.statusText,
          })
        }
      }
    }

    xhr.open('PUT', signedRequestUrl)
    xhr.send(file)
  })
}

// get the presigned url for a file
// and call makeS3Request to upload the file
async function uploadFileToS3(file: File, getSignedUrl: GetSignedUrlFunction): Promise<string> {
  try {
    const {fileUrl, signedRequestUrl} = await getSignedUrl(file)
    const url = await makeS3Request(fileUrl, signedRequestUrl, file)

    return url
  } catch (e) {
    alert('Could not upload file.')
    return '' // return empty string in case of error
  }
}

// check if a file is a video
function isVideoFile(file: File): boolean {
  if (file.type.slice(0, 5) === 'video') {
    return true
  }
  return false
}

// check file size in bytes against a max size in MB
function isTooBig(file: File, maxFileSize: number): boolean {
  const fileSize = file.size / 1024 / 1024 // in MB

  return fileSize > maxFileSize
}

// check file type
function isUnsupportedFileType(file: File, supportedFileTypes: string[]): boolean {
  return !includes(supportedFileTypes, file.type)
}

export default {
  uploadFileToS3,
  isVideoFile,
  isTooBig,
  isUnsupportedFileType,
} 