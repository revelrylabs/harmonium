...
describe('DatePicker', () => {
  ...
  it('should add className to child', () => {
    ...
    const input = shallow(<DatePicker className={testClassName} />)
      .find(`.${testClassName}`)
  ...
  it('can make items not selectable', () => {
    const spy = sinon.spy()
    const input = mount(<DatePicker defaultValue='2018-03-12' isOpen onChange={spy} isSelectable={(date) => date.day != 1}/>)
  ...
  it('handles a hash of highlights', () => {
    const input = mount(<DatePicker defaultValue='2018-03-12' highlights={{'2018-03-15': '__TEST__'}} isOpen />)

    expect(input.find('.__TEST__').exists()).to.equal(true)
  })

  it('handles a highlight function (that returns a class)', () => {
    const input = mount(<DatePicker defaultValue='2018-03-12' highlights={(date) => date.day % 2 == 0 ? '__TEST__' : ''} isOpen />)

    expect(input.find('.__TEST__').exists()).to.equal(true)
  })

  it('handles a highlight function (that returns a class)', () => {
    const input = mount(<DatePicker defaultValue='2018-03-12' highlights={(date) => date.day % 2 == 0} isOpen />)
