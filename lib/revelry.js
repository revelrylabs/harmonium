...
var Revelry = (function () {
  ...
  _createClass(Revelry, null, [{
    ...
    value: function viewToString(path, options) {
      ...
      var Main = Revelry.App.Components.Main;
...
Revelry.registerComponent('Main', (function (_React$Component) {
  ...
  _createClass(Main, [{
    ...
    key: 'pathToComponentClassName',

    /* CLASS METHODS */

    value: function pathToComponentClassName(path) {
      return path.split('/').map(_underscore2.default.str.classify).join('.');
    }
  }, {
    key: 'pathToComponentClass',
    value: function pathToComponentClass(path) {
      var parts = Main.pathToComponentClassName(path).split('.');
